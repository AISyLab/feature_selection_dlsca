...
if __name__ == "__main__":
    ...
    masks = mask_substitution[mask_shares]
    profiling_labels_masks = [
        [bin(AES_Sbox[int(p) ^ int(k)] ^ int(r)).count("1")] if leakage_model == "HW" else [AES_Sbox[int(p) ^ int(k)] ^ int(r)]
        for p, k, r in
        zip(np.asarray(profiling_plaintext[:, target_byte]), np.asarray(profiling_key[:, target_byte]),
            np.asarray(profiling_masks[:, target_byte]))]
