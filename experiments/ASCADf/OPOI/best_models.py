...
def best_cnn_id_opoi_700_ascadf(classes, number_of_samples):
    ...
    # Leakage model: ID
    # POI interval: [45400, 46100]
    # Number of parameters: 62396

    batch_size = 400
    tf.random.set_seed(271867)
    model = Sequential(name='best_cnn_id_opoi_ascadf_700')
    ...
    model.add(BatchNormalization())
    model.add(Conv1D(kernel_size=30, strides=15, filters=8, activation='selu', padding='same'))
    model.add(AveragePooling1D(pool_size=4, strides=4, padding='same'))
    model.add(BatchNormalization())
